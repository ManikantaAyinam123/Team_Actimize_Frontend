...
const PrivateRoute = ({ element: Component, allowedRoles }) => {
  ...
  const roles = JSON.parse(localStorage.getItem('data') || '[]'); // Assuming roles is stored as a JSON array
  const location = useLocation();

  const isAuthenticated = !!token;
  const isAuthorized = roles.some(role => allowedRoles.includes(role)); // Check if any role matches
